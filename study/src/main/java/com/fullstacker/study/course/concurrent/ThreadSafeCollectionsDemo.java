...
/**
 * @program: course
 * @description: 线程安全集合类的演示 怎么保证集合类的线程安全 1. 使用synchronized关键字 2. 使用Collections提供的工具方法保证线程安全，底层还是使用Synchronized关键字
 * Collections.synchronizedCollection(list) Collections.synchronizedList() Collections.synchronizedSet();
 * Collections.synchronizedMap(); 3. 使用juc包中的提供的集合类： CopyOnWriteArrayList CopyOnWriteArraySet ConcurrentHashMap
 * @author: xgs
